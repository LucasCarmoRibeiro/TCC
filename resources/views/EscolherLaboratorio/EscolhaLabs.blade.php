@extends('Bases.BaseMenuUsuario')

@section('conteudo')

<!--inicio conteudo-->


 <div class="container"> <!--inicio container card-->

        <div class="row">
            <div class="col-12">

<!--cards-->



@foreach ($laboratorios  as $item)






    <div class="col-1 card  divPC " style="width: 150px; background-color:#4A1B82;">

        <svg xmlns="http://www.w3.org/2000/svg" width="60" height="100" fill="currentColor" class="bi bi-pc-display icones " viewBox="0 0 16 16">
            <path fill-rule="evenodd" d="M8 1a1 1 0 0 1 1-1h6a1 1 0 0 1 1 1v14a1 1 0 0 1-1 1H9a1 1 0 0 1-1-1V1Zm1 13.5a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0Zm2 0a.5.5 0 1 1 1 0 .5.5 0 0 1-1 0ZM9.5 1a.5.5 0 0 0 0 1h5a.5.5 0 0 0 0-1h-5ZM9 3.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5ZM1.5 2A1.5 1.5 0 0 0 0 3.5v7A1.5 1.5 0 0 0 1.5 12H6v2h-.5a.5.5 0 0 0 0 1H7v-4H1.5a.5.5 0 0 1-.5-.5v-7a.5.5 0 0 1 .5-.5H7V2H1.5Z"/>
          </svg>

        <div class="card-body">
    <a href="{{ route('receberLaboratorio',['id' => $item->id ]) }}">  
         <p class="card-text ">{{$item->name}}</p> </a>
        </div>
    </div> <!--fim cards-->

    
    @endforeach

</div>


</div><!--fim container card-->
</div>


@endsection
